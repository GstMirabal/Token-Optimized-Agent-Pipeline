...
| **Domain** | `gate_transcription` | Owns writes of Phase 7 gate verdicts into `SPRINT_LOG.md` (`config/artifact_registry.json` role Orchestrator). Gates emit; this profile transcribes — `qa_agent` and `tester_agent` hold no Write/Edit (`F-026-A1`). |

...
| **Phase 2** | `staffing_injection` | Takes the Initial Roadmap (Work rows arrive with `Assignee (proposed)` from Phase 1) and assigns the most hyper-specialized subagent node for each step. The proposal is input, not a lock: this profile may overwrite it. The row is closed only when `agent_assignment.md` records the assignee (`docs/standards/templates/AGENT_ASSIGNMENT_TEMPLATE.md`). |

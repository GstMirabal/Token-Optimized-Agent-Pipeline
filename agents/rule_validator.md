...
| **Phase 2** | `task_scope` | Generates `task_scope.md` inside the sprint directory named in `agents.md §5 mandatory_topology`, **versioned**. It is not a session artifact: `jurisdictional_lock` and `no_interference` are applied by reading it, `loop_guard.py` measures progress from it, and `close_workflow.md` Phase 2.6 demands it as phase evidence. **Table shape**: `# \| File \| Operation \| Risk \| Assignee \| Model \| Effort \| Status` — Model/Effort are required under **every** harness once `sprint_id >= 28` (`scripts/check_task_scope.py:38` `MODEL_FROM_SPRINT`), not only under `session_tool: cursor`; the short `# \| File \| Operation \| Risk \| Assignee \| Status` shape is historical only, for sprints before 28. Enforced by `scripts/check_task_scope.py` (Sprint 030 / `F-026-A2`). One structural subject per subtask (`jurisdictional_lock`, restated Sprint 048 — a mandatory companion such as a `fix(` commit's paired test is not a second subject); a subject claimed by an in-progress subtask is locked for every other subtask (`no_interference`). |

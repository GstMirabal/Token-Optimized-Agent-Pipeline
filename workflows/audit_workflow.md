...
| **1. Topo Sweep** | `skill_standard_check`| Verify all skills in `skills/` legally adhere to the Three-File Skill Standard — run `python3 skills/topology-monitor/scripts/legacy_app_auditor.py`. |
...
| **2. Doc Purity** | `nomenclature` | `Doc Orchestrator` lists every file outside `docs/sprints/[Sprint_ID]-[Stack]-[Layer]/` (`agents.md §5 mandatory_topology`) as a rename candidate for human approval (`agents.md §2 destructive_flags`). |
| **2. Doc Purity** | `precision_audit` | Run the SkillOpt rules-consistency pass **only on explicit human authorization**, per `rules/skills_and_integrations.md §3 "SkillOpt train_runner.py — Canonical Invocation"`, with `--skill agents.md`. Done-criterion: exit `0` and a written diff of proposed edits recorded on this step — no edit is applied here. If the `skillopt` stack is not provisioned, record `precision_audit skipped: skillopt stack absent` and continue. |
| **2. Doc Purity** | `link_audit` | Ensure no internal `.md` file points to eradicated paths. Run `python3 skills/slash-commander/scripts/verify_commands.py` so every `commands/*.md` reference resolves and `docs/guides/AGENTS_SLASH_COMMANDS_GUIDE.md` §3.2 names each command stem. |
| **3. Verdict** | `report` | Generate a conclusive `docs/audits/PIPELINE_AUDIT_REPORT-[Sprint_ID].md` artifact (`RA-06` Option B; live convention e.g. `PIPELINE_AUDIT_REPORT-042.md`) detailing discrepancies neutralized. |

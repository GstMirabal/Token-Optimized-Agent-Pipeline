...
| # | Action |
| :--- | :--- |
| 1 | Run `python3 scripts/session_start.py` (or `make session-start`). Prints a ≤80-line briefing (anchor, drift summary, upstream open-count). No network. No `.env`. No full UPSTREAM dump. |
| 2 | Read that briefing. On drift exit `2`, run `/agents:reconcile` before handoff. |
| 3 | Execute the binding steps below (claim → probes → sync → bridge), then hand to **Principal Agent** for pipeline Phase 1. |
...
| **0.4 Drift** | `drift_check` | **Before `state_claim`.** `python3 .agents/scripts/detect_drift.py` — verdict, not boolean (ADR-0002). Against sealing tags (`## [X.Y.Z]`): **`S`** covered → `0`; **`M`/`U`/`A`/`R`** → `2` → `/agents:reconcile` before Phase 2. Orphaned baseline (squash-merge) falls back to merge-base and says so. No baseline yet → reports and passes. |
...
| **0.6 Probe** | `readiness_probe` | `python3 .agents/scripts/session_probe.py` — **advisory only**. Flags stale graph, missing docs, **anchor sprint ≠ checked-out `ai-sprint/[ID]`** (branch, not newest `docs/sprints/` dir). Gaps accepted by humans live in `acknowledged_gaps`. May propose `/agents:revdoc` (needs `triple_lock`). |
...
| **1.5 Bridge** | `bridge_check` | Run `.agents/scripts/install.sh --target claude` if `session_tool` is `claude-code`, or `--target cursor` if `cursor`, when **this target's** lock is missing (`.bridge_claude.lock` / `.bridge_cursor.lock`), lock commit ≠ `git -C .agents rev-parse HEAD`, or linked artifacts under `.claude/` / `.cursor/` are gone. Locks are independent per target. Host: `hooks/on_init.py` often covers this. **Nucleus:** no SessionStart hooks — run `scripts/install.sh --target …` when command symlinks are missing/stale vs `commands/`. Installer does not prune dangling symlinks. |
...
---
*Start v6.4.0 — briefing via `session_start.py`; pin sync auto-bumps to newest `v*` tag (`sync_agents_pin.py`).*

...
| **1.5 Bridge** | `bridge_check` | Run `.agents/scripts/install.sh --target claude` if `session_tool` is `claude-code`, or `--target cursor` if `cursor`, when **this target's** lock is missing (`.bridge_claude.lock` / `.bridge_cursor.lock`), lock commit ≠ `git -C .agents rev-parse HEAD`, **linked artifacts under `.claude/` / `.cursor/` are gone**, or (Cursor) `commands_stale()` — rendered `commands/` digests ≠ `.cursor/commands/` (Sprint 039). Locks are independent per target. Host: `hooks/on_init.py` often covers this. **Nucleus:** no SessionStart hooks — `--boot` runs install when stale. Installer does not prune dangling symlinks. |
...
---
*Start v6.5.0 — operator path = `session_start.py --boot`; pin sync auto-bumps to newest `v*` tag (`sync_agents_pin.py`); bridge freshness includes command-body digests.*

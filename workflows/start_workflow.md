...
| **0. Amnestic Anchor** | `read_graph` | If the `graphify` binary is missing from the virtual environment, install it via `venv/bin/pip install graphifyy`. If `graphify-out/graph.json` is missing, build it by running `venv/bin/graphify update .`. If present, query the graph via MCP/CLI to align on boundaries. |

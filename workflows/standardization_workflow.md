...
| Other frameworks' files (`.windsurfrules`, `copilot-instructions.md`, …) | Proposed for archive in the snapshot | Never deleted without explicit human OK — they may contain learning worth triaging. **Exception:** `.cursor/` is generated by `scripts/install.py --target cursor` and is never archived — same treatment as `.claude/` (regenerable bridge output). |

...
| :--- | :--- | :--- |
| **0. Amnestic Anchor** | `read_constitution` | `Governance Learner` MUST zero memory and read `agents.md §4` before scanning any log. |
| **1. Strategic Extraction** | `signal_scan` | Scan the current sprint's telemetry (`memory/telemetry/raw_errors.json`) and session logs for recurring friction, distinguishing "High-Signal" heuristics (worth keeping) from one-off noise. |

...
| **3. Roadmap Drafting** | `orchestrator` | Drafts Initial Roadmap and instantiates `docs/sprints/[ID]-[Stack]-[Layer]/`. **Extracts Phase 1's plan into that directory as `IMPLEMENTATION_PLAN.md` and commits it** — this is the earliest point at which the canonical path exists, which is why Phase 1 authors the plan and Phase 3 files it. **Also creates and checks out branch `ai-sprint/[ID]` from the base branch** (RA-12 Branch Discipline) — no commit may happen before this branch exists. **Deliverable**: `SPRINT_LOG.md` in that directory. |
| **4.1 Agent Assignment** | `agent_orchestrator` | Assigns the most specialized existing subagent to each roadmap step, or authors a new profile under `agents/` when none fits. **Deliverable**: `agent_assignment.md` in the sprint directory — every step has a named assignee. **A session that cannot dispatch subagents still writes it**, recording which profile's ruleset governed each write; that is the artifact's content, and a declared solo session is not an exemption from producing it. |
| **4.2 Skill Assignment** | `skill_architect` | Checks whether a computational tool already exists in `skills/` before each task starts; forges one under the Three-File Standard when it does not. **Deliverable**: `skill_assignment.md` in the sprint directory — every step has its tools resolved, and the skills **deliberately not used** are recorded beside those used. Both halves are the deliverable: a tool that was considered and rejected is a decision, a tool that was never seen is a gap. |
| **4.3 Rule Audit** | `rule_validator` | Audits the roadmap against current `rules/`, using the graphify dependency graph. **Deliverable**: `task_scope.md` inside the sprint directory named in `agents.md §5 mandatory_topology` — `# \| File \| Operation \| Risk \| Assignee \| Status`, which is the shape sprints 021 through 025 actually produce; the four-column form declared here until Sprint 023 named neither `Operation` nor `Risk` and matched no file on disk. It said *"at the host root"* and *"git-ignored"* until Sprint 023; Sprint 024 moved it into the sprint directory and made it versioned, and `scripts/docs_freshness_check.py` and `close_workflow.md` Phase 2.6 have read it there ever since. |
...
| **8. Sprint Closeout**| `principal_agent` | Update Blueprints, Global Roadmap, Walkthroughs, and the Master Ledger (host `CHANGELOG.md` `[Unreleased]` entry), including final Graph Rebuild via `make -f .agents/Makefile graphify-rebuild`. **Deliverable**: `PHASE_REGISTER.md` in the sprint directory — which phases ran and what each left. Then hand off to `workflows/close_workflow.md`. |

...
- **Next / in flight:** **`050` (`deterministic-quality-instrument`) closed, deploying (2026-09-24)** — built `scripts/quality_audit.py` (Python-only complexity auditor, closes `KI-049-4`'s Python half) and `session_state.py set-topology` (`topology_version` writer). The JS/TS half was built, gated, and withdrawn — three remediation rounds each found a new silent-compliance defect in a stdlib-only scanner; four consecutive `REJECTED`/`charter` gate verdicts escalated to `remediation_workflow.md`; human invoked the plan's own pre-declared Abort criterion 1 rather than ship a defective guarantee. `agents.md §1` now states plainly that no JS/TS instrument exists. Re-planned as its own sprint: `KI-050-6` below, with the five failure families both gates found as its acceptance-test corpus. 11 units + 3 abort units landed; `make verify` green, 805 tests. Section below. **`049` (`cursor-bridge-100`) deployed `v4.32.0` (PR #82, 2026-09-20)** — audited the Cursor integration bridge end to end at explicit human request ("verify it is 100% effective"); found and fixed six defects, five reproduced in an isolated sandbox before being touched (`F-049-1`..`F-049-5`), plus `F-049-7` — two governance files falsely credited two skills with computing a style-score neither computes, caught by `token_economy_agent`'s Phase 5 pre-approval audit before a human signature landed on the false premise, corrected instead of reclassified. 11 planned units landed (`U9` withdrawn at the hold, identifier retained per `RA-14`); `make verify` green, 780 tests pass (748 + 32 new). Real deterministic Python/JS quality auditor routed to `KI-049-4`, Sprint 050, below. Phase 7: QA Gate 1 `REJECTED`/`instructing` r1 (`agents.md` falsely claimed `ruff check .`/`pnpm run lint` "enforced by `make verify`" — 0 hits measured; a function past the 50-line limit; a stale Verdict paragraph) → `RECORD`/`testifying` r2 after remediation (`refactor(`, behaviour-preserving); Tester Gate 2 `RECORD`/`testifying` both rounds, round 2 proving the Gate-1 remediation behaviour-preserving by executing pre-/post-refactor code side by side over 7 stubbed scenarios rather than trusting a green suite. `docs_freshness_check` `BLOCK` resolved via `PIPELINE_AUDIT_REPORT-049.md`; `last_audit_sprint` advanced to 49. Section below. **`048` (`jurisdictional-lock-reconciliation-and-047-residue`) deployed `v4.31.0` (PR #81, 2026-09-18)** — restated `agents.md §2 jurisdictional_lock` from a bare file-count proxy to its invariant (one structural subject per task, disjoint between concurrently in-progress tasks), removing the contradiction with `rules/code_craft.md §6` that made Sprint 046 mislabel a `fix(` commit and Sprint 047 re-pair 7 units mid-execution; propagated across 6 derived statements (`RA-14`). Applied the five findings routed out of 047 (`KI-047-1, 3, 4, 5, 6`) plus one independent `RA-14` defect in `agents/rule_validator.md`. 14 planned units, 13 landed (U9 contingency not triggered, 0 fallout); `make verify` green, 748 tests pass. `KI-047-3` closed only in its anchor-resolution half — the sibling `check_invocation_coverage` extension surfaced 59 undeclared files, 5.9× Abort criterion 2's threshold, routed to `KI-048-1` below. Phase 7: QA Gate 1 `REJECTED`/`charter` r1 (task_scope.md never transcribed; U8's mid-execution rescope not reflected across the sprint's own artifacts; no destination for the deferred 59-item finding) → `RECORD`/`testifying` r2 after remediation; Tester Gate 2 `RECORD`/`testifying` (every modified/added test proven non-vacuous by mutation). One concurrent-agent `.git/index` race during execution, fully recovered in-session (`KI-048-2`). Section below. **`047` (`nucleus-audit-design-remediation`) deployed `v4.30.0` (PR #80, 2026-09-15)** — closed the design-pass bucket of the Sprint 045 audit (`S045-02, 06, 09, 14, 17, 18, 19, 20, 22, 27`) + `KI-045-2` + `KI-046-3` + the submodule-mode anchor-*read* findings + `submodule_purity --ignored`. 23 planned + 6 discovered mid-execution = 29 units, `make verify` green, 747 tests pass. Phase 7: QA Gate 1 `REJECTED`/`charter` r1 (sprint-directory records still claimed 23 units after 29 landed — `RA-14`'s own new sprint-artifact-set clause, added by this sprint's `U1`, violated by this sprint) → `RECORD`/`testifying` r2 (records-only fix, `IMPLEMENTATION_PLAN.md` untouched); Tester Gate 2 `RECORD`/`testifying` (one ambient-checkout-shape test coupling recorded, routed to Extract). A fresh `PIPELINE_AUDIT_REPORT-047.md` was run at close (`docs_freshness_check` `BLOCK` — structural delta since `last_audit_sprint=45` spanned both 046 and 047; `last_audit_sprint` advanced to 47). Section below. **`046` (`nucleus-audit-mechanical-remediation`) deployed `v4.29.0` (PR #79, 2026-09-08)** — applied the 18-row mechanical `S045-*` bucket + `KI-045-1` (Quality Gate table stub in `SPRINT_LOG_TEMPLATE.md`), one atomic commit per file, drafted text only, `agents.md §7` IDs `RA-01`..`RA-18` not renumbered (`RA-04`/`RA-10` → tombstone/pointer in place). Phase 7: QA Gate 1 `REJECTED`/`instructing` r1 (F-046-QA1 — a faithfully transcribed but defective drafted amendment `A4`) → `APPROVED` r2; Tester Gate 2 `RECORD`/`testifying` (T-046-1 closed by `U29` `tests/test_state_mirror.py`; T-046-2 pre-existing). **`045` (`nucleus-ruleset-mechanism-audit`) deployed `v4.28.0` (PR #78)** — full obsolescence audit; 29 `nucleus` rows bucketed 046/047/defer, analysis-only. Both Sprint 045 buckets are now closed. **`044` deployed** `v4.27.0`, **`043` deployed** `v4.26.0`, **`042` deployed** `v4.25.0`, **`041` deployed** `v4.24.0`.
...
| **T4** | In nucleus mode the `docs_freshness_check` `BLOCK` prescribes refreshing `0_SYSTEM_OVERVIEW.md` / `0_SYSTEM_ARCHITECTURE.md`, which are **host-only by design and absent here**; and `current_sprint.last_audit_sprint`, the field the whole check pivots on, is written by no workflow and no script (`grep -rn "last_audit_sprint" workflows/ commands/` → nothing) | Found at Sprint 042's close, which the `BLOCK` stopped. Resolved for that sprint by actually running `/agents:audit` (`docs/audits/PIPELINE_AUDIT_REPORT-042.md`) rather than incrementing an unowned field, but the gate will stop the next nucleus close the same way. Needs a nucleus-mode remedy the message can name, and an owner for the field |
...
### ✅ Taken by Sprint 048 (`jurisdictional-lock-reconciliation-and-047-residue`) — routed out of Sprint 047 (Extract, all `routing_class: nucleus`)

Five findings from Gate 1/Gate 2 and the sprint's own retrospective, none applied
in 047 (out of its declared scope; each is small enough for a future mechanical
bucket rather than its own sprint). `KI-047-2` is not a missing sixth row here —
it was absorbed into the `RA-14` amendment Sprint 047 itself already applied
(`docs/sprints/047-core-pipeline/SPRINT_LOG.md:88`), so it correctly has no row
of its own. `KI-047-1` recurs across sprints (Sprint
...
All five rows below — `KI-047-1`, `KI-047-3`, `KI-047-4`, `KI-047-5`, `KI-047-6`
— were applied by Sprint 048 (`jurisdictional-lock-reconciliation-and-047-residue`).
...
| `KI-047-3` | `verify_references.py` check (d)'s new `#anchor` fragment resolution (`S045-22`, Sprint 047) found 2 pre-existing broken `invoked_by:` anchors the moment it shipped, the same defect class as `S045-13` — both fixed inside Sprint 047 itself (`docs/sprints/047-core-pipeline/SPRINT_LOG.md:51-52`, units U25 and U26), so no anchor backlog exists. The actual gap is checker **coverage**: check (d) has two halves sharing the same non-recursive tree glob (`workflows/*.md`, `scripts/*.py`, `hooks/*.py`) — `check_invoked_by_anchors` (`#anchor` resolution, ~line 390) and `check_invocation_coverage` (`invoked_by:` presence, lines 234-236) — both missing `skills/*/scripts/*.py` and `tests/*.py` | ✅ Applied by Sprint 048, **anchor-resolution half only** (`jurisdictional-lock-reconciliation-and-047-residue`, unit U8 + paired test, `docs/sprints/048-core-pipeline/IMPLEMENTATION_PLAN.md`) — `check_invoked_by_anchors` now covers `skills/*/scripts/*.py` and `tests/*.py` (0 new findings). `check_invocation_coverage`'s sibling extension was attempted and reverted: it surfaced 59 files with no `invoked_by:` declaration in those trees (mostly ordinary pytest suites and skill helpers never individually required to declare one), over Abort criterion 2's 10-item threshold. That 59-item question is routed below under *Still open for a later program — routed out of Sprint 048* |
...
### Still open for a later program — routed out of Sprint 048 (`jurisdictional-lock-reconciliation-and-047-residue`)

One finding, deferred by the plan's own Abort criterion 2 rather than applied.

| # | Finding | Proposed destination |
| :--- | :--- | :--- |
| `KI-048-1` | `verify_references.py` check (d)'s `check_invocation_coverage` half (lines 234-236) requires every mechanism under its three globbed trees to declare `invoked_by:` or a typed exception. Extending its tree list to `skills/*/scripts/*.py` and `tests/*.py` (alongside the sibling `check_invoked_by_anchors` extension Sprint 048 did land) surfaces **59 files** with no `invoked_by:` declaration and no exception — measured directly during Sprint 048 U8, and 59 ≫ the plan's Abort criterion 2 threshold of 10, so the extension was not applied | Requires deciding first whether every `tests/*.py` and `skills/*/scripts/*.py` file should individually declare `invoked_by:` (most are ordinary pytest suites or skill helper modules, not standalone mechanisms) or whether `config/invocation_exceptions.json` should carry a blanket, typed exception for one or both trees — a policy question, not a mechanical fix, and sized for its own sprint (59 files) rather than a contingency slot |
...
| # | Finding | Proposed destination |
| :--- | :--- | :--- |
| `KI-049-4` | **Delivered (Python), Sprint 050 — `scripts/quality_audit.py`, `make quality-audit`.** JS/TS was built, gated, and withdrawn (Abort criterion 1); see `KI-050-6` below. Original text: `config/invocation_exceptions.json` names `skills/python-quality-auditor` and `skills/js-standardizer` as the instrument behind `agents.md §1`'s Python/JS style-score, `max_indentation` and `max_lines_per_func` rows. Measured during Sprint 049 (`F-049-7`): neither script computes a numeric score, checks indentation depth, measures function length, or exits non-zero | Delivered: stdlib `ast`-based Python auditor, `sys.exit(2)` on violation, standalone `make quality-audit` target (not wired into `verify` — baseline had pre-existing violations, `D5` Branch B, remediation routed to Sprint 051) |
| `KI-050-6` | `scripts/quality_audit.py`'s JS/TS scanner was built in Sprint 050 and withdrawn after four consecutive `REJECTED`/`charter` gate verdicts: three remediation rounds each fixed one silent-compliance defect and surfaced another; the last two (a method named after a JS control keyword; a regex literal desyncing the scanner's string-masking) were present since the instrument's first commit, not remediation regressions. JS/TS's lexical grammar is context-sensitive (regex-literal-vs-division, keyword-vs-identifier) in a way a brace-depth scanner without a real lexer cannot resolve in general | Re-plan JS/TS complexity measurement as its own sprint. Evaluate a real lexer as a justified dependency (`rules/code_craft.md §7`) or an explicitly bounded subset. Acceptance-test corpus already exists: the five failure families found by hand in Sprint 050's Phase 7 rounds 2-3 (arrow over-detection, unenclosed module-level callbacks, nested-arrow depth loss, control-keyword method names, regex-literal masking desync) |
...
## Appendix — Sprint 030 (`token-economy-enforcement`), delivered

...
| `KI-047-3` | `verify_references.py` check (d)'s new `#anchor` fragment resolution (`S045-22`, Sprint 047) found 2 pre-existing broken `invoked_by:` anchors the moment it shipped, the same defect class as `S045-13` — both fixed inside Sprint 047 itself (`docs/sprints/047-core-pipeline/SPRINT_LOG.md:51-52`, units U25 and U26), so no anchor backlog exists. The actual gap is checker **coverage**: `scripts/verify_references.py` check (d) globs only `workflows/*.md`, `scripts/*.py`, `hooks/*.py` non-recursively (lines 234-236 and 386-391), missing `skills/*/scripts/*.py` and `tests/*.py`, which also declare `invoked_by:` | ✅ Applied by Sprint 048 (`jurisdictional-lock-reconciliation-and-047-residue`) — unit U8 + paired test extend check (d) coverage to `skills/*/scripts/*.py` and `tests/*.py` (`docs/sprints/048-core-pipeline/IMPLEMENTATION_PLAN.md`) |

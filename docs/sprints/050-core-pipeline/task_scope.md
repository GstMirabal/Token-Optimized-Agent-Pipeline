...
| AB1 | `scripts/quality_audit.py` (+ paired `tests/test_quality_audit.py`, same commit) | modify — withdraws the JS/TS scanner (Abort 1, `IMPLEMENTATION_PLAN.md` `## Abort criterion` #1, human decision post-escalation) | medium | `implementer_agent` | sonnet | medium | ⏳ |

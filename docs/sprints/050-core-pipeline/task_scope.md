...
| U9 | `tests/test_audit_cursor_models.py` | modify | low | `implementer_agent` | sonnet | medium | ✅ `b48e924` |
| U10 | `docs/guides/WORKFLOWS_STEP_MAP_GUIDE.md` | modify (generated, never hand-edited) | low | `implementer_agent` (regenerated by the session directly — mechanical, `python3 scripts/map_workflows.py`, no authorship judgment) | sonnet | low | ✅ `45d557f` |
| AB1 | `scripts/quality_audit.py` (+ paired `tests/test_quality_audit.py`, same commit) | modify — withdraws the JS/TS scanner (Abort 1, `IMPLEMENTATION_PLAN.md` `## Abort criterion` #1, human decision post-escalation) | medium | `implementer_agent` | sonnet | medium | ✅ `25c0d5b` |
| AB2 | `agents.md` | modify — reverts §1 JS/TS complexity claim to "no instrument exists", cites `KI-050-6` not a sprint number; preserves D1/D2 unit-of-measure definitions, the lexer-honesty prohibition, and the F7 verify-gating disclosure | medium | `rule_validator` (done by the session directly — mechanical revert, no adversarial gate round needed for a deletion) | sonnet | medium | ✅ `ada2e14` |
| AB3 | `config/invocation_exceptions.json` | modify — `js-standardizer` note stops naming `quality_audit.py` as a JS/TS instrument, cites `KI-050-6` | low | `implementer_agent` (done by the session directly) | sonnet | low | ✅ `ada2e14` |

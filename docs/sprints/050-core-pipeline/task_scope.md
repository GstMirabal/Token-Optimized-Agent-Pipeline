...
| AB2 | `agents.md` | modify — reverts §1 JS/TS complexity claim to "no instrument exists", cites `KI-050-6` not a sprint number; preserves D1/D2 unit-of-measure definitions, the lexer-honesty prohibition, and the F7 verify-gating disclosure | medium | `rule_validator` | sonnet | medium | ⏳ |
| AB3 | `config/invocation_exceptions.json` | modify — `js-standardizer` note stops naming `quality_audit.py` as a JS/TS instrument, cites `KI-050-6` | low | `implementer_agent` | sonnet | low | ⏳ |

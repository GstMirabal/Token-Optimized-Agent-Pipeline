...
| :--- | :--- | :--- |
| A `fix(` commit must stage its regression test (`code_craft.md §6`), which forced pairing each Wave-2 script fix with its Wave-3 test inline rather than as a trailing group — the plan's original 3-wave design assumed sequential groups | Amend the pipeline/plan template guidance: a unit whose commit type will be `fix(` should be planned as one paired (impl + test) unit from Phase 1, not split across Work-table groups | `KI-047-1` (candidate, Extract to classify `routing_class`) |
| Drafted Sprint 045 amendment texts required D1 re-verification against the current tree in 3 of ~7 spot-checked cases (stale `invocation_exceptions.json` line numbers, a non-existent `--eval-only` flag, a wrong synthesis-file citation) — confirms D1 is load-bearing, not a formality | `agents.md §7 RA-14` already covers this; no new rule needed, but a KI worth indexing as precedent alongside Sprint 046 `F-046-QA1` | `KI-047-2` (candidate) |
| `verify_references.py` check (d)'s new `#anchor` resolution immediately found 2 pre-existing broken anchors (`loop_guard`, `session_bound`) the moment it shipped — the same defect class as `S045-13`, suggesting other latent `invoked_by:#anchor` defects may exist elsewhere in the corpus this sprint did not scan | Recommend a one-time full-corpus `invoked_by:` audit now that check (d) can catch this class | `KI-047-3` (candidate) |
| F-047-QA5 (`map_workflows.py build()` 57 lines, over `§1`'s 50-line limit), F-047-QA6 (1 new `RUF100`; `repository_hardening_workflow.md` lost its `When` column), F-047-T1 (`tests/test_session_start.py` ambient-checkout-shape coupling, one-line fix) | Each is a small, precisely-located fix — not worth a dedicated remediation round; carry to Extract for `routing_class` and pick up in a future mechanical bucket | `KI-047-4`, `KI-047-5`, `KI-047-6` (candidates) |
...
| Tester Agent (Gate 2) | 1 | RECORD | testifying | Full suite `747 passed` exit 0 on two consecutive runs plus a third inside `make verify` (installer sandbox 6/6) — no flakiness. Zero regression proven at node level: all 704 pre-sprint test IDs (`a995d6f`) still collected at HEAD, none removed/skipped; 43 new nodes (8/10/6/4/10/5). All 7 paired units proven by mutation (revert the fixed script to `a995d6f` in a scratch copy, confirm its paired tests red: `_mode` 2, `check_gate_log` 5, `verify_references` 5, `map_workflows` 10, `submodule_purity` 8, `session_start` 6) — not by trusting commit messages. U11+U25/U26 anchor coupling verified live in a scratch copy. `RA-11` holds: `verify_references.py`/`check_gate_log.py`/`submodule_purity.py` all exit 2, never 1, on failure. Readiness green (`verify_references`, `map_workflows --check`, `check_task_scope`, `check_forge_ladder`, `check_gate_log`, `scan_workflow_determinism`, `check_template_gates` all exit 0). Tree clean, HEAD `72d82ba`. Recorded: **F-047-T1** — `tests/test_session_start.py:56 test_main_exits_zero_and_respects_line_cap` gained a hidden dependency on the ambient checkout shape via U14's `anchor_root()` (patches `repo_root`, not `is_nucleus`); reds `1 failed` only when the suite runs from a `.git`-less or submodule-pointer checkout — a position `agents.md §3 jurisdiction`/`strict_rule` already forbids for nucleus work. Green in every sanctioned context (nucleus clone, CI, separate clone, worktree). One-line fix (mirror U22's `is_nucleus` patch) routed to Extract with F-047-QA5/QA6. Does not increment the consecutive-rejection counter; `remediation_workflow.md` not triggered. |
...
**Next Phase**: Both gates closed (`RECORD`/`testifying`, no bounce) → Phase 8 Sprint Closeout

...
| :--- | :--- | :--- |
| Other Models gate-Task limit → in-session Double-Gate | `discard` | Same posture as 037/038; not new policy |
| Mid-sprint `detect_drift` exit 2 on `ai-sprint/*` (unsealed commits) | `discard` | Expected until deploy seals ledger |
| Agent sandbox false reds on pytest `git init` | `discard` | CE-5; unsandboxed verify green |

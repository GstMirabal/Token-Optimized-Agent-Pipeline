...
`agent_orchestrator` assigns **profiles** (`agents/*.md`), not models.

| Owns | Does **not** own |
| :--- | :--- |
| Which ruleset governs each unit | Which Cursor model / effort runs it |
| Deviations that change the assignee (`tests/` → `devops_agent`) | `tier_escalation` and Cursor tier proposals |

**Model / effort** are owned by `token_economy_agent` (`tier_ownership`,
`tier_escalation`) and transcribed into `task_scope.md` by `rule_validator`
(same pattern as Sprint 026 § Declared escalations). They are **not** columns
of this file.
...
| P2.2 | `Makefile` | modify | sequential / ruleset | `devops_agent` | `agents/devops_agent.md` |
...
4. **Correction 2026-08-25.** An earlier draft of this file wrongly added Model/Effort columns and filled Claude Code aliases (`haiku`/`sonnet`/`opus`). That is out of Phase 4.1 jurisdiction and wrong for a Cursor session — see `task_scope.md` § Failure analysis and § Cursor tier map.

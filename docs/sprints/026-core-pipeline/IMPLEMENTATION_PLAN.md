...
- **P5.2** — Operación: añadir a la tabla *Rule Contexts* de `agents.md §0` una frase que nombre `config/rule_triggers.json` como la forma legible por máquina de esa misma tabla y declare que la comprobación `(e)` las mantiene sincronizadas. **Done-criterion**: `python3 scripts/verify_references.py` sale `0`.
...
| :--- | :--- | :--- |
| G1.q | Veredicto QA sobre las unidades del Hito 1, registrado en `docs/sprints/026-core-pipeline/SPRINT_LOG.md` | `qa_agent` |
| G1.t | Veredicto Tester sobre las mismas, en el mismo fichero | `tester_agent` |
...
| M5 | Una sesión de Cursor, preguntada por una regla de `agents.md §2` que no está en el prompt, la cita correctamente | Pregunta directa en el chat de Cursor, respuesta contrastada con el fichero | Que `.cursor/rules/00-constitution.mdc` **realmente** carga la constitución. Sin esto el Hito 2 correría sin gobernanza y nada lo señalaría |
...
| A3 | `docs/sprints/026-core-pipeline/SPRINT_LOG.md` | modify | medium | `qa_agent` | — (es la puerta) | ⏳ |

- **P7** — Operación: añadir a `agents.md §3` una fila que declare que el contenido del núcleo alcanzado por symlink (`AGENTS.md` en la raíz de un host apuntando a `.agents/agents.md`) queda **FUERA** del alcance de las puertas documentales del host, y que nombre el mecanismo de exclusión: el host excluye la ruta del symlink en la configuración de su propia puerta. Razón que la fila debe recoger: `agents.md §3 strict_rule` prohíbe al host modificar ese contenido, y una puerta que exige arreglar lo que la ley prohíbe tocar es un rojo insalvable (finding `C5`, línea 713 del fichero de findings). **Done-criterion**: la fila existe, nombra la ruta y nombra el mecanismo; `python3 scripts/verify_references.py` sale `0`.
...
- **P4.4** — Operación: actualizar `config/model_tiers.json`: el bloque `_comment` declara que la columna `cursor` la deriva `scripts/audit_cursor_models.py`, y **por qué los tiers `gate` y `author` siguen con `"model": null`** — un hito de sprint no es historial probado. Se rellena únicamente lo que `P4.2` proponga y el humano acepte. **Done-criterion**: `python3 scripts/check_model_tiers.py` sale `0`; el `_comment` nombra la causa medida y no deja el `null` sin explicación.
- **A3 — el análogo del paso 7, y la puerta final del hito.** Operación: al terminar el Hito 2, ejecutar `python3 scripts/docs_freshness_check.py . 026` y la Phase 2.6 de `close_workflow.md` sobre `docs/sprints/026-core-pipeline/`, y registrar en `SPRINT_LOG.md` que ninguna de las dos señala artefacto ausente ni malformado. Después, la comprobación de indistinguibilidad: **un agente en contexto fresco recibe el directorio del sprint con la columna `Tool` de `SPRINT_LOG.md` ocultada, y se le pide particionar los artefactos entre las dos herramientas.** **Done-criterion**: la partición propuesta no coincide con la real por encima del azar, y el resultado se registra literal. **Si acierta, la portabilidad ha fallado** y la diferencia que la delató se nombra por fichero — es la formulación operativa de la línea 1094 (*"If the gate can notice the difference, portability has not been achieved"*).
...
| `python3 scripts/verify_references.py` | `0`. `RA-16`: `scripts/cursor_adapter.py`, `hooks/on_push.py` y `scripts/audit_cursor_models.py` declaran invocador | Puerta del Hito 1 y cierre |

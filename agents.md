...
| **Certification** | See `RA-05: SPRINT_CLOSEOUT` for the closeout artifact list (Blueprints, Global Roadmap, Walkthroughs, Master Ledger). Single definition kept in `§7` to avoid an always-loaded duplicate. |
| **Open upstream findings** | **Nucleus sessions only.** `docs/audits/UPSTREAM_FINDINGS_FROM_HOSTS.md` holds framework-class defects reported by hosts under `§4 feedback_upstream` — the ones `strict_rule` forbade the host from patching. Read it before planning nucleus work, because several are blocking a shipped mechanism rather than proposing a new one. Announced here rather than left to be discovered: a nucleus session has no `docs/0_SYSTEM_OVERVIEW.md` (`F-093-N1`; host-only, Entry Point above) and **does** keep `docs/active_state.json` (`§5 state_anchor`; `close_workflow.md` Phase 4 applies in nucleus mode too). Start from this document and that anchor. |
...
| `rules/token_economy.md` | Authoring/auditing a plan or `task_scope`, measuring session cost, or changing model tiers / token-economy instruments (not every source edit). |
...
| **Python** | `linter_command` | `ruff check .` (Reject if exit code > 0). Verified by: QA-gate judgment, NOT `make verify` — `verify` runs no `ruff check` step (`grep -n 'ruff check' Makefile` → 0 hits). `skills/python-quality-auditor` is a separate, model-invoked tool call (`config/invocation_exceptions.json:53`) that shells to `ruff`/`mypy`/`bandit`/`radon` and prints a pass/fail line per tool; it computes no numeric score and never exits non-zero, so "Warn if score < 95" named an instrument this script does not implement (`F-049-7`, Sprint 049). Corrected here rather than dropped: a real deterministic style-score is routed to Sprint 050. |
| **JS/TS** | `naming_convention` | `camelCase` |
| **JS/TS** | `linter_command` | `pnpm run lint` (Reject if exit code > 0). Verified by: QA-gate judgment, NOT `make verify` — `verify` runs no `pnpm run lint` step. `skills/js-standardizer` is a separate, model-invoked tool call (`config/invocation_exceptions.json:58`) that checks for the presence of lint-tool config files and a single repo-wide boolean — whether *any* `.js`/`.ts` file anywhere contains the substring `@param` or `@returns`, not per-function JSDoc coverage; it computes no numeric score and never exits non-zero (`F-049-7`, Sprint 049). A real deterministic style-score is routed to Sprint 050. |
...
| **Complexity** | `max_indentation` | 3 levels, measured as **block-nesting depth counted over `ast` ancestors** (`FunctionDef`, `AsyncFunctionDef`, `ClassDef`, `If`, `For`, `While`, `With`, `Try`, `Match`) from the module root, not over character columns — a module-level `def` body is level 1, and the limit is exceeded at level 4. **No instrument exists for this row today** — deterministic or model-invoked. `skills/python-quality-auditor` and `skills/js-standardizer` were previously cited here; neither script reads indentation depth (`F-049-7`, Sprint 049, measured: `grep -c -i indent` on both scripts → 0). `make verify` runs no indentation check. A real instrument is routed to Sprint 050. |
| **Complexity** | `max_lines_per_func` | 50 lines, measured in **executable lines** — statements in the function body, excluding blank lines, comment-only lines, and the docstring — not raw span. **No instrument exists for this row today** — deterministic or model-invoked. `skills/python-quality-auditor` and `skills/js-standardizer` were previously cited here; neither script measures function length (`F-049-7`, Sprint 049, measured: `grep -c -i 'max_lines\|lines_per'` on both scripts → 0). `make verify` runs no per-function line-count check. A real instrument is routed to Sprint 050. |
...
| **Isolation** | `jurisdictional_lock` | A subagent task claims **exactly one file as its structural subject** — the file the task exists to change — declared in `task_scope.md`, and no concurrently in-progress task may claim that same subject (`no_interference`). **The subject is the unit of isolation, not a count of files in a commit**: a commit MAY stage further files that the subject's own rules require — notably the paired test a `fix(` commit must carry (`rules/code_craft.md §6`, enforced by `hooks/on_commit.py audit_regression_test`) — because a mandatory companion is not an independently claimable subject and cannot collide with another task. It caps **concurrent** claim scope, never lifetime touches on a file: a later task may take the same subject once the earlier one has landed (Phase 014 `T21`/`T22` precedent). Stated as a bare file count until Sprint 048. That bare count left the invariant unstated and produced the opposite reading downstream — that the mandatory test of a `fix(` commit violated this rule. Sprint 046 escaped it by mislabelling a `fix(` commit `refactor(`, and Sprint 047 re-paired 7 units mid-execution. |
...
| Category | Rule (Key) | Value / Constraint (Value) |
| :--- | :--- | :--- |
| **Framework** | `federation` | Check `git remote`; isolate `.agents` purely as a *Git Submodule*. The sanctioned bridge into the host's `.claude/` is exclusively `.agents/scripts/install.py` (`install.sh` is a deprecation shim that forwards to it; symlinks + non-destructive JSON merge) — no other mechanism may inject content into the host's Claude Code configuration. |
...
| **Infrastructure**| `topological_order`| Skills flat inside `skills/`. Sub-layers (`core/`, `local/`) PROHIBITED. Illustrative project-family packs live under `profiles/[name]/` inside the submodule (e.g. `profiles/example-project/`). **Production profiles are never committed to the public nucleus** (`RA-15`): they live at a **host-controlled path** — convention `<host-root>/.agents-profile/` or a private directory the host names — and are linked via `scripts/install.py --profile-path <path>` (Sprint 028). The legacy `--profile <name>` flag resolves only `profiles/<name>` inside the submodule. |
| **Infrastructure**| `enforcement` | The official auditor for Three-File-Standard adherence is `skills/topology-monitor/scripts/legacy_app_auditor.py` (`workflows/audit_workflow.md` `skill_standard_check`, `Makefile` `verify`). `mass-standardizer` regenerates `skills/manifest_skills.json` — it is the manifest generator, not the adherence auditor. |
| **Infrastructure**| `secret_sovereignty`| See `RA-09: SECRET_SOVEREIGNTY` (canonical — adds the `Makefile` / subshell mechanism). BANNED from reading `.env` into memory; use environment export commands. |
...
| **Feedback Loop** | `feedback_upstream` | **Canonical statement of the learning flow.** `§3 jurisdiction` and `workflows/extract_workflow.md` both point here *and* carry a compressed restatement of the framework-class path — grep both on every patch to this row (`RA-14`). `§3 strict_rule` is the prohibition this flow routes around, not a pointer to it. Learning is a LIVING three-tier flow, never a dead end. Each candidate carries a `routing_class` (`extract_workflow.md` Phase 2 — `host` \| `profile` \| `nucleus` \| `discard`): **host-class** findings stay in the host's `memory_index.json`; **project-family-class** findings route to `profiles/[name]/` (rules/skills/docs) — for a real project, the host-controlled profile path installed with `scripts/install.py --profile-path`, never `profiles/` inside the public nucleus (`RA-15`); **framework-class** findings (would improve every host) MUST become a nucleus contribution — a rule-amendment proposal or fix PR to the `.agents` repo. **How framework-class learning reaches the nucleus without violating `§3 strict_rule`** (the apparent contradiction — a host cannot write into `.agents`, yet the framework must learn from hosts): the host session only *detects and drafts* — it assigns `routing_class: nucleus`, records the finding, and surfaces it to the human during `extract_workflow`; the PR itself is authored from a **separate clone** of the nucleus repository (`§3 jurisdiction`), a distinct act from the host's sprint and never a write into the `.agents` submodule tree (`scripts/submodule_purity.py` refuses a close that contaminates it). Every host session is a sensor for the shared brain; letting a framework-class lesson die in a local index is a governance violation. How-to: `docs/guides/SELF_IMPROVEMENT_GUIDE.md`. |
...
| **MCP Registry** | `discovery_manifest` | `.agents/claude/mcp.json` is the submodule's MCP server template; `install.py` (`install.sh` is a deprecation shim that forwards to it) merges it non-destructively into the host's `.mcp.json`, which is the actual discovery point Claude Code reads. |
...
| Category | Rule (Key) | Value / Constraint (Value) |
| :--- | :--- | :--- |
| **Amendment** | `RA-01: ENVIRONMENT_VIOLATION`| On an environment/tooling failure (missing venv, absent binary, broken lockfile) the agent MUST attempt one scoped autonomous fix through the isolated local environment (`§3 dependencies`) before escalating, UNLESS `rules/project_topology.md` or the Implementation Plan names that remediation human-only. Procedure: `workflows/remediation_workflow.md` for loop stalls (auto-invoked after >3 consecutive `REJECTED` on one logic block — `remediation_workflow.md` Phase 0); `RA-03: HOTFIX_FLAT` for CRITICAL/HIGH defects outside the sprint pipeline. Invoked by: `pipeline_workflow.md` Execution phase on any tool/interpreter failure, and `devops_agent` environment routines. |
| **Amendment** | `RA-02: LAZY_SIGNAL_PARADIGM` | See `rules/django_backend_standard.md §2 Signal Registration` (single authority since Sprint 047). Anchor kept resolvable for legacy `Clause J-02` via `rules/LEGACY_RULE_CONCORDANCE.md`. |
...
| **Amendment** | `RA-04: FULL_DEPLOYMENT` | **Retired Sprint 046** — superseded by `§6 orchestrator` (roadmap author, not deployer) + `RA-12 BRANCH_DISCIPLINE` + `deployment_workflow.md` Phase 1 (the sole merge authority). Historical text: "Orchestrator MUST deploy the COMPLETE task hierarchy in a single atomic action" — 0 external citations across 45 sprints, no implementing workflow, and it contradicted the `§6 orchestrator` charter. Row kept as a tombstone: `RA-05`..`RA-18` are **never renumbered** (100+ external by-number citations). |
...
| **Amendment** | `RA-10: SUPPLY_CHAIN_SHIELD`| See `§8 Supply Chain Security` (canonical). `§8` is the superset: the same `pnpm 11+` / `ignore-scripts=true` / `minimum-release-age=1440` plus `onlyBuiltDependencies`. Row kept as a pointer so the external `RA-10` citations stay resolvable; `RA-05`..`RA-18` are never renumbered. |
...
| **Amendment** | `RA-14: PATCH_PROPAGATION`| A long-lived planning/design document revised across multiple sessions or review rounds MUST, at each patch, be grepped in full for other mentions of the same term/field before the patch is considered closed. A fix applied only where a reviewer looked, while the same reference drifts uncorrected elsewhere in the same artifact, is not a correction — it is a new inconsistency. **Headline-metrics clause** (`KI-045-2`): a deliverable's headline metrics — verdict counts, cross-reference targets — MUST be a `grep`-reproducible figure derived from the body register, never carried from the author's summary prose. Precedent: Sprint 045 Gate 1 cited a skill that exists nowhere, and Gate 2 found a headline `27/9` against a register of `22/14`. **Sprint-artifact-set clause** (Sprint 044 proposal): the mandatory patch-propagation grep extends from "the same artifact" to the sprint's **artifact set** (Implementation Plan, `task_scope.md`, ADRs, skill-assignment and every sprint-directory document), and binds to the **remediation step**, not only the reviewer's spot-check. Precedent: Sprint 044 reconciled "403/404" → "403" across the plan, `task_scope.md` and `ADR-0014` but left a 4th spelling in `skill_assignment.md:38` until Gate 1 round 2. |
| **Amendment** | `RA-15: HOST_CONTENT_GENERICIZATION`| Any `feedback_upstream` (`agents.md §4`) contribution to the nucleus — a framework-class fix, skill, or test fixture discovered while working inside a real host project — MUST have every host-identifying string (real project name, real absolute filesystem paths, real business logic/thresholds) genericized before the PR. Real project profiles (`profiles/[name]/`) are never committed to the public nucleus at all — they live at a host-controlled path (convention `<host-root>/.agents-profile/` or equivalent; installed with `scripts/install.py --profile-path`). Precedent: a real host's absolute path leaked into `skills/skillopt/data/scenarios.json`, and a real host project name was hardcoded into `skills/compliance-checker/scripts/kill_switch.sh`, both found only during a pre-publication audit rather than caught at contribution time. |
...
| **Amendment** | `RA-17: GATE_VERDICT_CLASSES` | Phase 7 emits `APPROVED` \| `REJECTED` \| `RECORD` with class `charter` / `instructing` / `testifying`. `RECORD` does not count toward remediation. Procedure: `rules/qa_and_testing.md` §4. |
| **Amendment** | `RA-18: CURSOR_PHASE1_NO_PLAN_MODE` | Under `session_tool: cursor`, Phase 1 MUST NOT invoke `SwitchMode` to plan — that mode forbids writing the deliverable. Write `IMPLEMENTATION_PLAN.md` at the canonical sprint path in the current session. A file under `~/.cursor/plans/` is not the `triple_lock` plan. Procedure: `pipeline_workflow.md` Phase 1. |

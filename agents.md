...
| **Complexity** | `max_indentation` | 3 levels. **No instrument exists for this row today** — deterministic or model-invoked. `skills/python-quality-auditor` and `skills/js-standardizer` were previously cited here; neither script reads indentation depth (`F-049-7`, Sprint 049, measured: `grep -c -i indent` on both scripts → 0). `make verify` runs no indentation check. A real instrument is routed to Sprint 050. |
| **Complexity** | `max_lines_per_func` | 50 lines. **No instrument exists for this row today** — deterministic or model-invoked. `skills/python-quality-auditor` and `skills/js-standardizer` were previously cited here; neither script measures function length (`F-049-7`, Sprint 049, measured: `grep -c -i 'max_lines\|lines_per'` on both scripts → 0). `make verify` runs no per-function line-count check. A real instrument is routed to Sprint 050. |

...
| **Python** | `linter_command` | `ruff check .` (Reject if exit code > 0) + style-score audit via `skills/python-quality-auditor` (Warn if score < 95). Verified by: QA-gate judgment, NOT `make verify` — `verify` runs no `ruff check` / style-score step (`grep -n 'ruff check' Makefile` → 0 hits); the skill is declared model-invoked at `config/invocation_exceptions.json:53`. No `python-doctor` binary exists in the tree — the score is the `python-quality-auditor` output. Contrast the deterministic rows below (`path_type`, `ephemeral`, `code_logic`), which `make verify` does enforce. |
| **JS/TS** | `naming_convention` | `camelCase` |
| **JS/TS** | `linter_command` | `pnpm run lint` (Reject if exit code > 0) + style-score audit via `skills/js-standardizer` (Warn if score < 95). Verified by: QA-gate judgment, NOT `make verify` — `verify` runs no `pnpm run lint` / style-score step; the skill is declared model-invoked at `config/invocation_exceptions.json:58`. No `react-doctor` binary exists in the tree — the score is the `js-standardizer` output. |
...
| **Complexity** | `max_indentation` | 3 levels. Verified by: QA-gate judgment via `skills/python-quality-auditor` (`config/invocation_exceptions.json:53`) / `skills/js-standardizer` (`:58`), NOT `make verify` — `verify` runs no indentation check. |
| **Complexity** | `max_lines_per_func` | 50 lines. Verified by: QA-gate judgment via `skills/python-quality-auditor` (`config/invocation_exceptions.json:53`) / `skills/js-standardizer` (`:58`), NOT `make verify` — `verify` runs no per-function line-count check. |
